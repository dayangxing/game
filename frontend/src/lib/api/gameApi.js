...
export function createGameApi(options = {}) {
  ...
  async function loadDailyActionsWithState(game, view) {
    if (shouldUseBackend(game, canUseBackend)) {
      ...
      return {
        ...
        game: data.game ? withMode(data.game, 'api') : game
      };
    }
  ...
  return {
    ...
    async getCharacterPreview({ name, rerollSeed, attributes, mode = preferredMode } = {}) {
      if (mode === 'api' && canUseBackend) {
        const data = await requestJson({
          baseUrl,
          fetchImpl,
    ...
    async createFormalGame({ name, rerollSeed, attributes, mode = preferredMode } = {}) {
      if (mode === 'api' && canUseBackend) {
        const data = await requestJson({
          baseUrl,
          fetchImpl,
...
async function requestEventStream({ baseUrl, fetchImpl, path, method = 'GET', body, handlers = {} }) {
  ...
  while (true) {
    ...
    for (const part of parts) {
      ...
      if (event.name === 'narration_reset') {
        rawNarration = '';
        lastNarrationPreview = '';
        narrationPreviewComplete = false;
        handlers.onNarrationReset?.(event.data);
      }

      if (event.name === 'story_reset') {
        rawStory = '';
        lastStoryPreview = '';
        storyPreviewComplete = false;
        handlers.onStoryReset?.(event.data);
      }
