...
- 默认页签：`洞府`
- 顶部页签：`洞府`、`修炼`、`个人`、`秘境`、`行囊`
- `洞府`：展示命途状态、历史行为和今日行动，是唯一提供行动选择的主页。
- `修炼`：展示闭关修炼状态和当前瓶颈建议，不再放行动卡。
- `个人`：详细属性面板，集中展示人物、五维、生命与修行、宗门、道友牵绊和已修功法，不重复行囊里的宝物/丹药材料列表。
- `秘境`：展示雾隐秘境线索、天机事件和长期伏笔。
- `行囊`：专门展示奇珍法器、丹药与材料。
