...
AI 文字修仙 Web 原型。当前稳定版已经完成单机事件库 MVP：新手序章解释世界观，完成后创建随机属性角色，再进入状态驱动事件循环。前端负责桌面端交互和展示，后端负责权威存档、每日行动、回合结算与剧情生成 fallback，双方通过 REST JSON API 对接。产品不做开放世界、大地图、多人联机或离线挂机收益。
...
- 角色创建阶段只有点击 `开始修行` 才会调用 `POST /api/v1/game/new` 创建正式后端存档；`重掷` 仅更新本地预览 seed。
