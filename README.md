...
- 五维属性为 `rootBone`、`comprehension`、`fortune`、`willpower`、`lifeSeed`，分别对应根骨、悟性、福缘、心志、命种。
- 根骨影响基础体魄与部分突破成功率；悟性影响修炼理解与突破成功率；福缘影响随机机缘与少量突破加成；心志影响冲关稳定性；命种决定寿元上限并降低日常寿元消耗。
- 正式角色拥有 `health/maxHealth` 与 `lifespan/maxLifespan`。健康用于承受事件和突破反噬；寿元是每回合的正式行动成本，也是长期失败压力来源。
- 突破由后端先计算 `breakthroughPreview` 中的目标境界、成功率和失败代价，再落成实际成功或失败结果；前端与 LLM 只能展示它，不能重算它。
- 行囊中的 treasures 与已习得 techniques 都由后端规则授予并写入权威存档，它们会通过派生加成影响突破、体魄或修炼收益。
